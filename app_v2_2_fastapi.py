...
class TrackDetails(BaseModel):
    recent_tracks_details: List[Dict[str, Any]]
...
def prepare_upsert_data(recent_fetched_tracks):
    upsert_data = []
...
def check_and_upsert_data(recent_fetched_tracks, index=index):
    upsert_data = prepare_upsert_data(recent_fetched_tracks)
...
@app.post("/perform-tasks", response_model=SongInfo)
async def perform_tasks(recent_tracks_details):
    try:
        ...
        check_and_upsert_data(recent_fetched_tracks)

        played_song_ids = []
        for song_name in recent_fetched_tracks['Song ID']:
            played_song_ids.append(song_name)

        # song_id, features, popularity, duration, playcount, listeners, artist, album
        ...
        return{"Success":True, "Songs_info":songs_info}
    except Exception as e:
